...
BEGIN
    ...
    -- Verify normalized term-frequency table has entries for all 5 documents
    SELECT COUNT(*) INTO v_term_count
    FROM facets.bm25_index i
    JOIN facets.bm25_term_frequencies tf
      ON tf.table_id = i.table_id AND tf.term_hash = i.term_hash
    WHERE i.table_id = 'bm25_phase3_test.concurrent_docs'::regclass::oid
      AND i.term_text = 'databas';
