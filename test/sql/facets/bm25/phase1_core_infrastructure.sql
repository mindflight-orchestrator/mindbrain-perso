...
DECLARE
    v_frequency int;
BEGIN
    SELECT tf.frequency INTO v_frequency
    FROM facets.bm25_index i
    JOIN facets.bm25_term_frequencies tf
      ON tf.table_id = i.table_id AND tf.term_hash = i.term_hash
    WHERE i.table_id = (SELECT table_id FROM facets.faceted_table WHERE schemaname = 'bm25_phase1_test' AND tablename = 'documents')
      AND i.term_text = 'postgresql'
      AND tf.doc_id = 1
    LIMIT 1;
