...
const graph_entity_statement =
    ...
    \\);
;
const graph_entity_import_statement =
    \\CREATE TABLE IF NOT EXISTS graph_entity (
    \\    entity_id INTEGER PRIMARY KEY,
    \\    workspace_id TEXT NOT NULL DEFAULT 'default',
    \\    entity_type TEXT NOT NULL,
    ...
    \\);
;
const graph_entity_alias_statement =
    ...
    \\);
;
const graph_entity_alias_import_statement =
    \\CREATE TABLE IF NOT EXISTS graph_entity_alias (
    \\    term TEXT NOT NULL,
    \\    entity_id INTEGER NOT NULL,
    \\    confidence REAL NOT NULL DEFAULT 1.0
    \\);
;
const graph_relation_statement =
    ...
    \\);
;
const graph_relation_import_statement =
    \\CREATE TABLE IF NOT EXISTS graph_relation (
    \\    relation_id INTEGER PRIMARY KEY,
    \\    workspace_id TEXT NOT NULL DEFAULT 'default',
    \\    relation_type TEXT NOT NULL,
...
pub const metadata_schema_statements = [_][]const u8{
...
pub fn renderMetadataSchemaWithMode(allocator: std.mem.Allocator, mode: SchemaMode) ![]u8 {
    ...
    for (metadata_schema_statements, 0..) |statement, index| {
        if (mode == .import and shouldSkipImportStatement(statement)) continue;
        if (index > 0) try buf.appendSlice(allocator, "\n\n");
        if (mode == .import and std.mem.eql(u8, statement, graph_entity_statement)) {
            try buf.appendSlice(allocator, graph_entity_import_statement);
        } else if (mode == .import and std.mem.eql(u8, statement, graph_entity_alias_statement)) {
            try buf.appendSlice(allocator, graph_entity_alias_import_statement);
        } else if (mode == .import and std.mem.eql(u8, statement, graph_relation_statement)) {
            try buf.appendSlice(allocator, graph_relation_import_statement);
        } else {
            try buf.appendSlice(allocator, statement);
        }
    }
...
fn shouldSkipImportStatement(statement: []const u8) bool {
    return std.mem.eql(u8, statement,
        \\CREATE INDEX IF NOT EXISTS graph_entity_name_idx
        \\    ON graph_entity(name);
    ) or std.mem.eql(u8, statement,
        \\CREATE INDEX IF NOT EXISTS graph_entity_workspace_id_idx
        \\    ON graph_entity(workspace_id);
    ) or std.mem.eql(u8, statement,
        \\CREATE INDEX IF NOT EXISTS graph_relation_workspace_id_idx
...
test "sqlite schema includes the first standalone metadata tables" {
...
test "sqlite import schema skips graph indexes" {
