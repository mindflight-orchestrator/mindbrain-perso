...
pub const MindbrainHttpOptions = struct {
    addr_text: []const u8,
    db_path: []const u8,
...
pub const MindbrainHttpApp = struct {
    ...
    service_name_owned: []u8,
    default_workspace_id_owned: []u8,
    default_workspace_domain_json_owned: []u8,
    default_workspace_label_owned: ?[]u8,
    default_workspace_description_owned: ?[]u8,
    ...
    pub fn init(
        ...
        return try initWithOptions(allocator, io, .{
    ...
    pub fn initWithOptions(
        ...
        const static_dir_owned = try allocator.dupe(u8, options.static_dir);
        errdefer allocator.free(static_dir_owned);
        const service_name_owned = try allocator.dupe(u8, options.service_name);
        errdefer allocator.free(service_name_owned);
        const default_workspace_id_owned = try allocator.dupe(u8, options.default_workspace.id);
        errdefer allocator.free(default_workspace_id_owned);
        const default_workspace_domain_json_owned = try allocator.dupe(u8, options.default_workspace.domain_profile_json);
        errdefer allocator.free(default_workspace_domain_json_owned);
        const default_workspace_label_owned = if (options.default_workspace.label) |value|
            try allocator.dupe(u8, value)
        else
            null;
        errdefer if (default_workspace_label_owned) |value| allocator.free(value);
        const default_workspace_description_owned = if (options.default_workspace.description) |value|
            try allocator.dupe(u8, value)
        else
            null;
        errdefer if (default_workspace_description_owned) |value| allocator.free(value);
        ...
        var app = MindbrainHttpApp{
            ...
            .service_name_owned = service_name_owned,
            .default_workspace_id_owned = default_workspace_id_owned,
            .default_workspace_domain_json_owned = default_workspace_domain_json_owned,
            .default_workspace_label_owned = default_workspace_label_owned,
            .default_workspace_description_owned = default_workspace_description_owned,
    ...
    pub fn deinit(self: *MindbrainHttpApp) void {
        ...
        self.allocator.free(self.service_name_owned);
        self.allocator.free(self.default_workspace_id_owned);
        self.allocator.free(self.default_workspace_domain_json_owned);
        if (self.default_workspace_label_owned) |value| self.allocator.free(value);
        if (self.default_workspace_description_owned) |value| self.allocator.free(value);
    }
    ...
    pub fn serve(self: *MindbrainHttpApp) !void {
        ...
        log.info(
            "{s} listening on {s} ({s}, max_body_bytes={}, max_connections={})",
            .{
                self.service_name,
    ...
    fn initDatabase(self: *MindbrainHttpApp) !void {
        ...
        try workspace_sqlite.upsertWorkspace(
            ...
            self.default_workspace_domain_json_owned,
        );
        if (self.default_workspace_label_owned != null or self.default_workspace_description_owned != null) {
            try self.updateDefaultWorkspaceMetadata(db);
        }

        if (self.warn_on_empty_graph and try self.graphEntityCount(db) == 0) {
    ...
    fn updateDefaultWorkspaceMetadata(self: *MindbrainHttpApp, db: facet_sqlite.Database) !void {
        const c = facet_sqlite.c;
        const stmt = try facet_sqlite.prepare(
            db,
            "UPDATE workspaces SET label = COALESCE(?1, label), description = COALESCE(?2, description) WHERE id = ?3",
        );
        defer facet_sqlite.finalize(stmt);

        if (self.default_workspace_label_owned) |label| {
            try facet_sqlite.bindText(stmt, 1, label);
        } else if (c.sqlite3_bind_null(stmt, 1) != c.SQLITE_OK) {
            return error.BindFailed;
        }
        if (self.default_workspace_description_owned) |description| {
            try facet_sqlite.bindText(stmt, 2, description);
        } else if (c.sqlite3_bind_null(stmt, 2) != c.SQLITE_OK) {
            return error.BindFailed;
        }
        try facet_sqlite.bindText(stmt, 3, self.default_workspace_id_owned);
    ...
    fn archiveLabQueueMessage(queue: *queue_sqlite.QueueStore, queue_name: []const u8, msg_id: i64) void {
        if (queue.archive(queue_name, msg_id)) |_| {} else |err| {
