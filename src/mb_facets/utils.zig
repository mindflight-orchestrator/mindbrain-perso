...
extern fn get_ts_config_oid_helper(config_name: [*c]const u8, missing_ok: bool) c.Oid;
extern fn to_tsvector_byid_helper(config_oid: c.Oid, text_datum: c.Datum) c.Datum;
...
/// Maximum number of lexemes allowed in any tsvector we consume natively.
/// PostgreSQL itself limits this far below 2^31, so a value past this hints
/// at a corrupted varlena and we refuse to parse it.
pub const MAX_TSVECTOR_LEXEMES: usize = 1 << 20;

/// Bounds info computed from a detoasted tsvector varlena.
/// `str_area_len` is the number of bytes available for lexeme strings and
/// position blocks, derived from VARSIZE rather than from any in-band field.
pub const TSVectorBounds = struct {
    str_area_len: usize,
    num_lexemes: usize,
};

/// Validate the structural layout of a detoasted tsvector against its real
/// VARSIZE before any pointer arithmetic happens. Returns null when the
/// header / WordEntry array would overflow the actual buffer or when the
/// declared lexeme count is unreasonable. Callers must treat a null return
/// as "do not touch this varlena" and bail out gracefully.
pub fn tsvectorBounds(tsv: [*c]c.struct_varlena) ?TSVectorBounds {
    if (tsv == null) return null;

    const total_signed = varsize(tsv);
    if (total_signed <= 0) return null;
    const total: usize = @intCast(total_signed);
    if (total < @sizeOf(TSVectorData)) return null;

    const tsv_data = @as(*TSVectorData, @alignCast(@ptrCast(tsv)));
    if (tsv_data.size < 0) return null;
    const num_lexemes: usize = @intCast(tsv_data.size);
    if (num_lexemes > MAX_TSVECTOR_LEXEMES) return null;

    const word_entries_bytes = num_lexemes * @sizeOf(WordEntry);
    const header_and_entries = @sizeOf(TSVectorData) + word_entries_bytes;
    if (header_and_entries > total) return null;

    return TSVectorBounds{
        .str_area_len = total - header_and_entries,
        .num_lexemes = num_lexemes,
    };
}

/// Bounds-checked equivalent of POSDATALEN. Returns 0 when the position
/// vector header would land outside the validated string area, which is
/// safer than dereferencing a wild pointer.
pub fn safePOSDATALEN(
    tsv: [*c]c.struct_varlena,
    entry: *align(1) const WordEntry,
    str_area_len: usize,
) u32 {
    if (entry.haspos == 0) return 0;
    const offset = @as(usize, entry.pos) + @as(usize, entry.len);
    const aligned_offset = (offset + 1) & ~@as(usize, 1);
    if (aligned_offset > str_area_len) return 0;
    if (str_area_len - aligned_offset < @sizeOf(u16)) return 0;
