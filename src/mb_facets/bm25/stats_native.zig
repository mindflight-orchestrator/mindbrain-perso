...
pub fn bm25_term_stats(fcinfo: c.FunctionCallInfo) callconv(.c) c.Datum {
    ...
    // nentry = sum of all normalized term frequencies
    const query = std.fmt.allocPrintSentinel(allocator,
        \\SELECT 
        \\    i.term_text,
        \\    rb_cardinality(i.doc_ids)::bigint as ndoc,
        \\    COALESCE(SUM(tf.frequency), 0)::bigint as nentry
        \\FROM facets.bm25_index i
        \\LEFT JOIN facets.bm25_term_frequencies tf
        \\  ON tf.table_id = i.table_id AND tf.term_hash = i.term_hash
        \\WHERE i.table_id = {d}
        \\GROUP BY i.term_hash, i.term_text, i.doc_ids
    ...
    // Avoid backend abort if econtext is unexpectedly null.
    const econtext_term = rsi.econtext orelse {
        utils.elog(c.ERROR, "bm25_term_stats: ReturnSetInfo.econtext is null");
        return 0;
    };
    const oldcontext = c.MemoryContextSwitchTo(econtext_term.*.ecxt_per_query_memory);
...
pub fn bm25_doc_stats(fcinfo: c.FunctionCallInfo) callconv(.c) c.Datum {
    ...
    const econtext_doc = rsi.econtext orelse {
        utils.elog(c.ERROR, "bm25_doc_stats: ReturnSetInfo.econtext is null");
        return 0;
    };
    const oldcontext = c.MemoryContextSwitchTo(econtext_doc.*.ecxt_per_query_memory);
    rsi.setResult = c.tuplestore_begin_heap(true, false, utils.workMem());
...
pub fn bm25_explain_doc(fcinfo: c.FunctionCallInfo) callconv(.c) c.Datum {
    ...
    // Get collection stats through repository adapter
    const repository = bm25_pg_adapter.Adapter.asRepository();
    const collection_stats = repository.getCollectionStatsFn(repository.ctx, allocator, table_id) catch {
        utils.elog(c.ERROR, "Failed to load BM25 collection statistics");
        return c.PointerGetDatum(null);
    };
    const total_docs: f64 = @floatFromInt(collection_stats.total_documents);
    const avgdl: f64 = if (collection_stats.avg_document_length > 0.0) collection_stats.avg_document_length else 1.0;

    // Get document length
    var doc_len_query_buf: [256]u8 = undefined;
    const doc_len_query = std.fmt.bufPrintZ(&doc_len_query_buf, "SELECT doc_length FROM facets.bm25_documents WHERE table_id = {d} AND doc_id = {d}", .{ table_id, doc_id }) catch {
        utils.elog(c.ERROR, "Buffer too small");
        return c.PointerGetDatum(null);
    };

    var doc_length: f64 = 0;
    var ret = c.SPI_execute(doc_len_query.ptr, true, 1);
    if (ret == c.SPI_OK_SELECT and c.SPI_processed > 0) {
    ...
    const query = std.fmt.allocPrintSentinel(allocator,
        ...
        \\FROM facets.bm25_index i
        \\JOIN facets.bm25_term_frequencies tf
        \\  ON tf.table_id = i.table_id AND tf.term_hash = i.term_hash
        \\WHERE i.table_id = {d}
        \\  AND tf.doc_id = {d}
        \\ORDER BY tf DESC, i.term_text
    , .{ table_id, doc_id }, 0) catch {
        utils.elog(c.ERROR, "Failed to allocate query");
        return c.PointerGetDatum(null);
    };
    defer allocator.free(query);
    ...
    const econtext_explain = rsi.econtext orelse {
        utils.elog(c.ERROR, "bm25_explain_doc: ReturnSetInfo.econtext is null");
        return 0;
    };
    const oldcontext = c.MemoryContextSwitchTo(econtext_explain.*.ecxt_per_query_memory);
    rsi.setResult = c.tuplestore_begin_heap(true, false, utils.workMem());
...
pub fn bm25_collection_stats(fcinfo: c.FunctionCallInfo) callconv(.c) c.Datum {
    ...
    const econtext_coll = rsi.econtext orelse {
        utils.elog(c.ERROR, "bm25_collection_stats: ReturnSetInfo.econtext is null");
        return 0;
    };
    const oldcontext = c.MemoryContextSwitchTo(econtext_coll.*.ecxt_per_query_memory);
    rsi.setResult = c.tuplestore_begin_heap(true, false, utils.workMem());
