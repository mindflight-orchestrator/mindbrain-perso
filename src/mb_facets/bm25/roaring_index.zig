...
pub fn datumToRoaringBitmap(datum: c.Datum) !*c.roaring_bitmap_t {
    utils.elogFmt(c.DEBUG3, "[TRACE] datumToRoaringBitmap: Starting deserialization", .{});
    
    // Step 1: Detoast the datum (may return pointer to tuple memory or a copy)
    utils.elogFmt(c.DEBUG3, "[TRACE] datumToRoaringBitmap: Step 1 - Calling detoast_datum", .{});
    const varlena = utils.detoast_datum(datum);
    const varlena_ptr = @as(*align(1) c.struct_varlena, @ptrCast(varlena));
    utils.elogFmt(c.DEBUG3, "[TRACE] datumToRoaringBitmap: Step 1 - detoast_datum returned varlena_ptr={*}", .{varlena_ptr});
    
    // Step 2: Read header IMMEDIATELY (before any potential memory invalidation)
    utils.elogFmt(c.DEBUG3, "[TRACE] datumToRoaringBitmap: Step 2 - Reading header", .{});
    ...
    utils.elogFmt(c.DEBUG3, "[TRACE] datumToRoaringBitmap: Step 2 - Header read: total_size={d}, varhdrsz={d}, len={d}", .{ total_size, varhdrsz, len });
    
    // Step 3: Read data pointer (may point to tuple memory)
    utils.elogFmt(c.DEBUG3, "[TRACE] datumToRoaringBitmap: Step 3 - Getting data pointer", .{});
    const data = @as([*]u8, @ptrCast(varlena_ptr)) + varhdrsz;
    utils.elogFmt(c.DEBUG3, "[TRACE] datumToRoaringBitmap: Step 3 - Data pointer={*}, len={d}", .{ data, len });
    
    // Step 4: Copy ALL data to stable memory IMMEDIATELY
    // This must happen before SPI_finish() or any memory context changes
    utils.elogFmt(c.DEBUG3, "[TRACE] datumToRoaringBitmap: Step 4 - Allocating stable memory (len={d})", .{len});
    ...
    utils.elogFmt(c.DEBUG3, "[TRACE] datumToRoaringBitmap: Step 4 - Copying data from {*} to {*}", .{ data, data_copy_ptr });
    @memcpy(data_copy_ptr[0..len], data[0..len]);
    utils.elogFmt(c.DEBUG3, "[TRACE] datumToRoaringBitmap: Step 4 - Data copied successfully", .{});
    ...
    utils.elogFmt(c.DEBUG3, "[TRACE] datumToRoaringBitmap: Step 5 - Deserializing bitmap from copied data", .{});
    ...
    utils.elogFmt(c.DEBUG3, "[TRACE] datumToRoaringBitmap: Step 5 - Bitmap deserialized successfully, bitmap={*}", .{bitmap});
    
    // Free data_copy now that bitmap has its own copy
    utils.elogFmt(c.DEBUG3, "[TRACE] datumToRoaringBitmap: Step 6 - Freeing data_copy", .{});
    std.c.free(data_copy);
    utils.elogFmt(c.DEBUG3, "[TRACE] datumToRoaringBitmap: Step 6 - Returning bitmap={*}", .{bitmap});
...
pub fn roaringBitmapToDatum(bitmap: *c.roaring_bitmap_t) c.Datum {
    utils.elogFmt(c.DEBUG3, "[TRACE] roaringBitmapToDatum: Starting, bitmap={*}, cardinality={d}", .{ bitmap, cardinality(bitmap) });

    const size = c.roaring_bitmap_portable_size_in_bytes(bitmap);
    const varhdrsz = @as(usize, @intCast(utils.varhdrsz()));
    const total_size = varhdrsz + size;
    utils.elogFmt(c.DEBUG3, "[TRACE] roaringBitmapToDatum: size={d}, varhdrsz={d}, total_size={d}", .{ size, varhdrsz, total_size });

    utils.elogFmt(c.DEBUG3, "[TRACE] roaringBitmapToDatum: Allocating varlena", .{});
    const varlena = @as(*c.struct_varlena, @ptrCast(c.palloc(@intCast(total_size))));
    utils.set_varsize(varlena, @intCast(total_size));
    utils.elogFmt(c.DEBUG3, "[TRACE] roaringBitmapToDatum: varlena allocated at {*}", .{varlena});

    const data = utils.vardata(varlena);
    utils.elogFmt(c.DEBUG3, "[TRACE] roaringBitmapToDatum: Serializing bitmap to data={*}", .{data});
    ...
    utils.elogFmt(c.DEBUG3, "[TRACE] roaringBitmapToDatum: Serialized, written={d}, expected={d}", .{ written, size });
    if (written != size) {
    ...
    utils.elogFmt(c.DEBUG3, "[TRACE] roaringBitmapToDatum: Returning Datum (bitmap can now be freed)", .{});
    return datum;
}
