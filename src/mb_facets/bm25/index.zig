...
pub fn indexDocument(table_id: c.Oid, doc_id: i64, text: []const u8, config_name: []const u8, allocator: std.mem.Allocator) !void {
    ...
    for (tokens.items) |token| {
        if (custom_stopwords.contains(token)) continue;
...
fn updateInvertedIndex(table_id: c.Oid, term_hash: i64, term_text: []const u8, doc_id: i64, roaringbitmap_oid: c.Oid, term_freq: i32, config_name: []const u8, allocator: std.mem.Allocator) !void {
    ...
    const freq_query = try std.fmt.allocPrintSentinel(allocator,
    ...
    , .{ table_id, term_hash, doc_id, term_freq }, 0);
    defer allocator.free(freq_query);

    const freq_ret = c.SPI_execute(freq_query.ptr, false, 0);
    if (freq_ret != c.SPI_OK_INSERT and freq_ret != c.SPI_OK_UPDATE) {
        utils.elog(c.ERROR, "Failed to upsert BM25 term frequency");
        return error.UpsertFailed;
    }
}
...
fn updateDocumentMetadata(table_id: c.Oid, doc_id: i64, doc_length: i32, config_name: []const u8, allocator: std.mem.Allocator) !bool {
    ...
    const check_query = try std.fmt.allocPrintSentinel(allocator, "SELECT 1 FROM facets.bm25_documents WHERE table_id = {d} AND doc_id = {d}", .{ table_id, doc_id }, 0);
...
fn updateStatisticsIncremental(table_id: c.Oid, new_doc_length: i32, is_new_doc: bool, allocator: std.mem.Allocator) !void {
    if (is_new_doc) {
        ...
        const update_query = try std.fmt.allocPrintSentinel(allocator,
            ...
            \\    last_updated = now()
        , .{ table_id, new_doc_length, new_doc_length }, 0);
...
pub fn recalculateStatistics(table_id: c.Oid, allocator: std.mem.Allocator) !void {
    if (c.SPI_connect() != c.SPI_OK_CONNECT) {
        utils.elog(c.ERROR, "SPI_connect failed");
        return error.SPIConnectFailed;
    }
...
fn recalculateStatisticsInternal(table_id: c.Oid, allocator: std.mem.Allocator) !void {
    // Calculate new statistics from scratch
    const stats_query = try std.fmt.allocPrintSentinel(allocator, "SELECT COUNT(*)::bigint, COALESCE(AVG(doc_length), 0)::float FROM facets.bm25_documents WHERE table_id = {d}", .{table_id}, 0);
    ...
    if (c.SPI_processed > 0) {
        ...
        const update_query = try std.fmt.allocPrintSentinel(allocator, "INSERT INTO facets.bm25_statistics (table_id, total_documents, avg_document_length) VALUES ({d}, {d}, {d}) ON CONFLICT (table_id) DO UPDATE SET total_documents = EXCLUDED.total_documents, avg_document_length = EXCLUDED.avg_document_length, last_updated = now()", .{ table_id, total_docs, avg_len }, 0);
        defer allocator.free(update_query);
...
pub fn deleteDocument(table_id: c.Oid, doc_id: i64, allocator: std.mem.Allocator) !void {
    if (c.SPI_connect() != c.SPI_OK_CONNECT) {
        utils.elog(c.ERROR, "SPI_connect failed");
        return error.SPIConnectFailed;
    }
    ...
    const update_query = try std.fmt.allocPrintSentinel(allocator, "UPDATE facets.bm25_index SET doc_ids = rb_remove(doc_ids, {d}) WHERE table_id = {d} AND rb_contains(doc_ids, {d})", .{ doc_id, table_id, doc_id }, 0);
    defer allocator.free(update_query);

    _ = c.SPI_execute(update_query.ptr, false, 0);

    const delete_freq_query = try std.fmt.allocPrintSentinel(allocator, "DELETE FROM facets.bm25_term_frequencies WHERE table_id = {d} AND doc_id = {d}", .{ table_id, doc_id }, 0);
    defer allocator.free(delete_freq_query);

    _ = c.SPI_execute(delete_freq_query.ptr, false, 0);

    // Clean up empty terms
    const cleanup_query = try std.fmt.allocPrintSentinel(allocator, "DELETE FROM facets.bm25_index WHERE table_id = {d} AND rb_is_empty(doc_ids)", .{table_id}, 0);
    defer allocator.free(cleanup_query);

    _ = c.SPI_execute(cleanup_query.ptr, false, 0);

    // Delete document metadata
    const delete_doc_query = try std.fmt.allocPrintSentinel(allocator, "DELETE FROM facets.bm25_documents WHERE table_id = {d} AND doc_id = {d}", .{ table_id, doc_id }, 0);
