...
pub fn bm25_index_worker_native(fcinfo: c.FunctionCallInfo) callconv(.c) c.Datum {
    ...
    var custom_stopwords = stopwords.loadWithExistingConnection(language, allocator) catch {
        utils.elogWithContext(c.ERROR, "bm25_index_worker_native", "Failed to load BM25 stopwords");
        return c.PointerGetDatum(null);
    };
