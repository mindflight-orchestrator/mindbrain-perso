...
pub fn tokenizeToHashes(text: []const u8, allocator: std.mem.Allocator) !std.ArrayList(i64) {
...
test "tokenizePure basic" {
    ...
    var tokens = try tokenizePure("Hello World", allocator);
    defer {
        for (tokens.items) |token| {
            allocator.free(token);
        }
...
test "tokenizePure with stop words" {
    ...
    var tokens = try tokenizePure("the quick brown fox", allocator);
    defer {
        for (tokens.items) |token| {
            allocator.free(token);
        }
...
test "tokenizeToHashes" {
    ...
    var hashes = try tokenizeToHashes("laptop computer", allocator);
